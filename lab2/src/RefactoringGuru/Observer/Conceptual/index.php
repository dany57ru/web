...
class Subject implements \SplSubject
{
    ...
    public function detach(\SplObserver $observer): void
    {
        ...
        echo "Издатель: Отстраненный наблюдатель.\n";
    }
    ...
    {
        echo "Издатель: Уведомление наблюдателей...\n";
    ...
    {
        echo "\nИздатель: Я делаю кое-что важное.\n";
        $this->state = rand(0, 10);

        echo "Издатель: Мое состояние только что изменилось на
: {$this->state}\n";
        $this->notify();
    }
}
...
class ConcreteObserverA implements \SplObserver
{
    ...
    {
        if ($subject->state < 3) {
            echo "ConcreteObserverA: Отреагировал на событие.\n";
        }
    }
}
...
class ConcreteObserverB implements \SplObserver
{
    public function update(\SplSubject $subject): void
    {
        if ($subject->state == 0 || $subject->state >= 2) {
            echo "ConcreteObserverB: Отреагировал на событие.\n";
        }
    }
}
