...
class MonkeyHandler extends AbstractHandler
{
    ...
    {
        if ($request === "Банан") {
            return "Обезьяна: Я съем это " . $request . ".\n";
        } else {
...
class SquirrelHandler extends AbstractHandler
{
    ...
    {
        if ($request === "Орех") {
            return "Белка: Я съем это " . $request . ".\n";
        } else {
            return parent::handle($request);
        }
    }
...
class DogHandler extends AbstractHandler
{
    public function handle(string $request): ?string
    {
        if ($request === "Фрикадельки") {
            return "Собака: Я съем это " . $request . ".\n";
        } else {
            return parent::handle($request);
        }
    }
...
function clientCode(Handler $handler)
{
    foreach (["Орех", "Банан", "Чашка кофе"] as $food) {
        echo "Клиент: Кто хочет получить " . $food . "?\n";
        $result = $handler->handle($food);
        if ($result) {
            echo "  " . $result;
        } else {
            echo "  " . $food . " был оставлен нетронутым.\n";
        }
    }
}
