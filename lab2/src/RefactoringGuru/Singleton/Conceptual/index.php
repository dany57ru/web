...
class Singleton
{
    ...
    {
        throw new \Exception("Не удается отменить сериализацию синглтона.");
    }
...
function clientCode()
{
    ...
    if ($s1 === $s2) {
        echo "Работает синглтон, обе переменные содержат один и тот же экземпляр.";
    } else {
        echo "Произошел сбой в работе синглтона, переменные содержат разные экземпляры.";
    }
}
